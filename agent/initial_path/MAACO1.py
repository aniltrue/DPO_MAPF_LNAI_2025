...
from .aco import optimize, get_heuristic_matrix, update_pheromone
from agent.AbstractAgent import AgentID
from agent.Path import Path
from threading import Thread


class MAACO1(AbstractInitialPathStrategy):
    """
        This Pheromone-Based MA-ACO decreases the pheromone levels where the other agents pass
    """
    ...
    def generate_initial_paths(self, scenario: AbstractMap, agent_ids: List[AgentID], t_max: Dict[AgentID, int],
        ...
        for i in range(max_iteration):  # Start iteration
            ...
            # Update pheromones based on the other best paths
            ...
            for agent_id in agent_ids:
                ...
                if len(other_best_paths) > 0:
                    pheromones[agent_id] = update_pheromone(pheromones[agent_id], numba.typed.List(other_best_paths),
                                                            t_max[agent_id], inverse=True)
