...
class ACOWAPRepairingStrategy(AbstractRepairingStrategy):
    """
    ...
    @property
    def name(self) -> str:
        return "ACO-Based WAP Repairing Strategy"
        ...
        for i in range(self.MAX_ITER):  # Start iteration
            ...
            if len(other_best_paths) > 0:
                cost, aco_path, updated_pheromone = optimize(map_data, pheromones, heuristics, agent.t_max - t,
                                                             numba.typed.List(other_best_paths), position, beta=beta,
                                                             alpha=1.0, number_of_ants=self.NUMBER_OF_ANTS,
                                                             weighted_average=True)
            else:
                cost, aco_path, updated_pheromone = optimize(map_data, pheromones, heuristics, agent.t_max - t,
                                                             None, position, beta=beta, alpha=1.0,
