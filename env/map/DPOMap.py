...
from env.utils import check_for_collision, move_dynamic_obstacles, update_vision


class DPOMap(AbstractMap):
    """
    ...
    def __init__(self, n: int):
        ...
        if n != -1:
            self._data = np.zeros((n, n, 5), dtype=np.int32)

    def initiate(self):
        """
            This method initiates the *vision*
        """

        update_vision(self._data)

    def step(self, **kwargs):
        """
            This method plays *dynamic obstacles* and updates *vision*
        """
    ...
    def check_collision(self, **kwargs) -> bool:
        if "masked" in kwargs and kwargs["masked"]:
            return check_for_collision(self._data, [0, 3], mask_axis=4)
        else:
            return check_for_collision(self._data, [0, 3])

    def check_data(self) -> bool:
        return super().check_data() and self._data.shape[2] == 5
    ...
    def get_data(self) -> np.ndarray:
        """
            Convert *Dynamic Obstacles* to *Static Obstacles* if it is **observable**

            :return: Converted Map Data
        """
        data_clone = self._data.copy()
        update_vision(data_clone)
        data_clone[:, :, 3] *= (1 - data_clone[:, :, 4])

        data_clone_result = self._data.copy()

        data_clone_result[:, :, 0] += (1 - data_clone[:, :, 0]) * (data_clone[:, :, 3] > 0) * 2 * (1 - data_clone[:, :, 4])
    ...
    def random_map_factory(n: int, number_of_agents: int, density: float, seed: Optional[int],
        ...
        DPOMap.random_dynamic_obstacles(map_obj, nodes, number_of_agents, rnd, number_of_dynamic_obstacles)
