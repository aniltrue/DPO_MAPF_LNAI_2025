...
from env.map.StaticMap import AbstractMap
from env.utils import check_for_collision, update_vision


class PartialObservableMap(AbstractMap):
    """
    ...
    def check_collision(self, **kwargs) -> bool:
        if "masked" in kwargs and kwargs["masked"]:
            return check_for_collision(self._data, [0], mask_axis=3)
        else:
    ...
    def clone(self) -> AbstractMap:
        map_obj = PartialObservableMap(-1)
    ...
    def random_map_factory(n: int, number_of_agents: int, density: float, seed: Optional[int],
        ...
        map_obj = PartialObservableMap(n)
